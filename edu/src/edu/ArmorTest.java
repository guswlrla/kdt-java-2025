...
class ArmorTest {
	...
	public static void main(String[] args) {
		String st;
		st = "hello"; // 스트링 리터럴은 메소드 영역의 constant pool에 저장
		st.toString();
		
		int result = sum(10, 20);
		Armor am; // am은 로컬변수로서 참조변수
		am = new Armor(); // 생성자 호출
		// 아래 코드는 무엇이 문제인가? - 참조하는 변수를 써야함
//		new Armor();
//		new Armor();
//		new Armor();
		Armor am2 = new Armor();
		...
		int n = 10;
		// Armor a; //클래스로 변수 선언시 초기화가 필요하다.(참조변수만 있고 가리키는게 없어서 에러 발생)
		Armor a = new Armor();
		 ...
		 *  
		 */
		suit1 = null; // NullPointerException => 객체가 없어서 호출할 수 없음
		suit1.setName("mark6");
		suit1.setHeight(180);
