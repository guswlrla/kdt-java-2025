...
class Item { // 제품
	...
	@Override // object 클래스의 toString()을 오버라이드
	public String toString() {
		return name + ", 단가 : " + price;
	}
}
...
class Order {
	...
	public Order(Customer customer, int size) {
		...
		items = new Item[size]; // size는 최대 제품 개수
		quantities = new int[size]; // 주문 개수를 저장하는 배열
		this.count = 0;
	}
	...
	public void printOrderSummary() {
		// 객체.메소드() - 앞에 객체를 써야함, 같은 클래스 내에서는 객체 안써도 됨. this.calculateTotal() 이런식으로 쓸 수 있는데 this는 생략 가능 
		// customer가 객체도 아닌데 왜 customer.toString()을 쓸 수 있냐면
		// Order 생성자에서 customer는 Customer타입의 참조변수
		// boy를 받고, 그 boy는 객체를 생성하기 때문에 가능
