...
class SeasonalDiscount implements Discountable {
	...
	public SeasonalDiscount(double discountRate) {
		seasonalDiscountRate = discountRate; // 변수명이 다를 땐 this 안써줘도 됨
	}
	...
	public double getDiscountedPrice(double price) {
		double seasonDisprice = price * getSeasonalDiscountRate();
		return seasonDisprice;
	}
}
...
class Order3 extends SeasonalDiscount {
	...
	// 생성자
	public Order3(Customer3 regularCustomer, double discountRate) {
		super(discountRate);
	...
	public void printDiscountDetails() {
		 ...
		 */
		double seasonPrice = calculateTotal() - getDiscountedPrice(calculateTotal())
				- calculateTotal() * customer.getDiscountRate();
		System.out.println("시즌가격 : " + seasonPrice);
		System.out.println("=".repeat(55));
	}
}
...
class RegularCustomer3 extends Customer3 {
	...
	double applyDiscount(double totalAmount) {
		// 일반 고객은 추가 할인 없음
		double disAmount = totalAmount * getDiscountRate();
