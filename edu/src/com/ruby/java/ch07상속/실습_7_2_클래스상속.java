...
class Order2 {
	...
	// 생성자
	public Order2(PremiumCustomer premiumCustomer, int size) {
		this.customer = premiumCustomer;
		items = new Item2[size];
		quantities = new int[size];
		this.itemCount = 0;
	}

	public Order2(RegularCustomer regularCustomer, int size) {
	...
	private double calculateDiscountedTotal() {
		double disTotal = calculateTotal() - customer.applyDiscount(calculateTotal());
		return disTotal;
	}
