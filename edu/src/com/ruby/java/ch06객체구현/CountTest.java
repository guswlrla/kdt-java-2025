...
public class CountTest { //file 이름과 같아야 한다
	...
	static void showStatic() {//239 페이지의 예제 코드, 객체가 안 만들어져도 사용 가능
		System.out.println("클래스 메소드");
	}
	public static void main(String[] args) {
		Count.print1(); // 클래스 메소드 호출
		// print1(); // 에러 - countTest클래스 외부에 있기 때문
		
		Count.totalCount++; // 클래스 필드의 사용 235페이지 (2)클래스 필드 사용
		// Count.count = 10; // 오류 - 인스턴트 필드 사용 
		
		// Count.count++; // 오류 발생(클래스.static 변수가 아닌 것)
		Count c = new Count(); // 이렇게 써야함
		c.count = 10;
		c.totalCount++; // 에러가 아니나 비추 - 경고 메시지
		
		CountTest.showStatic(); // 아래 코드와 같음
		showStatic(); // 클래스 내에 있으므로 "클래스."을 생략해도 됨
